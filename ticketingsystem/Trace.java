...
public class Trace {
	...
	final static int coachnum = 1; // coach is arranged from 1 to 5
	final static int seatnum = 100; // seat is allocated from 1 to 20
	final static int stationnum = 2; // station is designed from 1 to 5

	final static int testnum = 400;
	final static int retpc = 0; // return ticket operation is 10% percent
				...
				public void run() {
					...
					for (int i = 0; i < testnum; i++) {
						...
						if (0 <= sel && sel < retpc && soldTicket.size() > 0) { // return ticket
							...
							System.out.println(
									"RemainTicket" + " " + leftTicket + " " + route + " " + departure + " " + arrival);
							System.out.flush();
